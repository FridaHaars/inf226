    ...
    render(elt, html`
        ...
        <div class="data">
            ...
            <div class="more">
                ...
                ${format_field('Favourite colour', html`<div class="color-sample" style="${'background:' + user.color}">`)}
                ${format_field('About', user.about, 'long')}
            </div>
        </div>
        <div class="controls">
            ${window.current_user_id == user.id ? '' : html`<button type="button" class="button" data-user-id="${user.id}" data-action="add_buddy">Add buddy</button>`}
        </div>
    `);
    return elt;
}
